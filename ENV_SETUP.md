...
Django endi avval **loyiha root papkasidagi umumiy `.env` faylini** o'qiydi, agar topilmasa, `backend/.env` dan o'qiydi.

### 1. Loyiha root papkasida `.env` faylini yarating

Loyiha root papkasida (hr_bot/) `.env` faylini yarating va quyidagi sozlamalarni qo'shing:
...
# Backend API URL (telegram_bot uchun)
...
```
✅ Loading environment from root .env: D:\coding\hr_bot\.env
```
...
- Umumiy `.env` fayli barcha servislar (backend, telegram_bot) uchun ishlatiladi
